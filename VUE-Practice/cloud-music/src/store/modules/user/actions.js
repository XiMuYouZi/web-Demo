import storage from "good-storage";
import { userProfile } from "network/account";
import { LocalStorageKEY } from "@/utils";
import { notify, isDefined } from "@/utils";

export default {
    async login({ commit }, uid) {
        const error = () => {
            notify.error("登陆失败,请输入正确的uid。");
            return false;
        };
        if (!isDefined(uid)) {
            return error();
        }

        try {
            const user = await userProfile(uid);
            const { profile } = user;
            commit("setUser", profile);
            storage.set(LocalStorageKEY.USERINFO, profile);
        } catch (e) {
            return error();
        }

        // const { playlist } = await getUserPlaylist(uid)
        // commit('setUserPlaylist', playlist)
        return true;
    },
    logout({ commit }) {
        commit("setUser", {});
        commit("setUserPlaylist", []);
        storage.set(LocalStorageKEY.USERINFO, null);
    }
};
